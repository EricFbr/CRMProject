...
<body>
	...
	<div style="margin-left: 20px;margin-bottom: 40px">
		<div id="navigation" style="margin: 0 auto">
			<span style="font-size: 18;font-weight: bold;">查询类型:</span><select id="qtype" name="qtype"
				...
				<option value="customStatus">按客户状态查询</option>
	...
	<div id="cm-dlg" class="easyui-dialog" style="width:400px;padding-left: 10px" closed="true"
		...
		<form id="cm-ff" method="post">
			...
			<div>
				<p>
					<label for="customStatus" class="label-top">客户状态:</label><select id="customStatus"
						panelHeight="auto" class="easyui-combobox" name="customStatus" style="width:100px;"
						data-options="editable:false">
						<option value="0" selected="selected">新增未上门</option>
						<option value="1">新增已上门</option>
						<option value="2">销售跟进中</option>
		...
		<div id="cm-query" class="easyui-panel" closed="true" noheader="true" closable="true">
			...
			<table id="cm-dg" title="查询结果" singleSelect="true" toolbar="#cm-toolbar" autoSave="false"
				...
				{'education':'4','text':'硕士'}],valueField:'education',textField:'text',panelHeight:'auto'}}">教育水平</th>
						...
						<th field=customStatus width="100" formatter="statuFormat" align="center"
							editor="{type:'combobox',options:{data:[{'customStatus':'0','text':'新增未上门'},
				{'customStatus':'1','text':'新增已上门'},
				{'customStatus':'2','text':'销售跟进中'},
				{'customStatus':'3','text':'咨询跟进中'},{'customStatus':'4','text':'死单'},{'customStatus':'5','text':'已报名'}],valueField:'customStatus',textField:'text',panelHeight:'auto'}}">状态</th>
	...
	<script type="text/javascript">
		...
		function statuFormat(value, row) {
			if (row.customStatus == 0) {
				return '新增未上门';
			} else if (row.customStatus == 1) {
				return '新增已上门';
			} else if (row.customStatus == 2) {
				return '销售跟进中';
			} else if (row.customStatus == 3) {
				return '咨询跟进中';
			} else if (row.customStatus == 4) {
				return '死单';
			} else {
				return '已报名';
			}
		...
		function queryType() {
			...
			if (type != 'createDate' && type != 'customStatus' && type != 'all') {
