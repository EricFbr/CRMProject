...
<body>
		...
		<table id="cr-dg" title="客户跟踪" singleSelect="true" toolbar="#cr-toolbar" autoSave="false"
			...
			<thead>
				<tr>
					...
					<th field="consultStatus" width="100" formatter="consultStatusFormat" align="center"
						editor="{type:'combobox',options:{data:[{'consultStatus':'0','text':'新增'},
				{'consultStatus':'1','text':'紧跟'},
				{'consultStatus':'2','text':'已报名'},
				{'consultStatus':'3','text':'死单'},{'consultStatus':'4','text':'报名后退费'}],valueField:'consultStatus',textField:'text'}}">跟踪状态</th>
	...
	<script type="text/javascript">
		...
		/* 客户状态格式化方法 */
		function consultStatusFormat(value, row) {
			if (row.consultStatus == 0) {
				return '新增';
			} else if (row.consultStatus == 1) {
				return '紧跟';
			} else if (row.consultStatus == 2) {
				return '已报名';
			} else if (row.consultStatus == 3) {
				return '死单';
			} else
				(row.consultStatus == 4)
			{
